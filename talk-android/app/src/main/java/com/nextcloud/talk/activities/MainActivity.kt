...
class MainActivity :
    ...
    private fun handleIntent(intent: Intent) {
        ...
        // Handle deep links for call URLs: https://nc.softblinktech.com/call/TOKEN
        if (intent.action == Intent.ACTION_VIEW && intent.data != null) {
            val url = intent.data.toString()
            val currentUser = currentUserProvider.currentUser.blockingGet()
            
            if (currentUser != null && UriUtils.isInstanceInternalTalkUrl(currentUser.baseUrl!!, url)) {
                Log.d(TAG, "Deep link detected: $url")
                val roomToken = UriUtils.extractRoomTokenFromTalkUrl(url)
                val bundle = Bundle()
