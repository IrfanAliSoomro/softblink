...
import com.nextcloud.talk.adapters.CallHistoryAdapter
...
class ConversationsListActivity :
    ...
    enum class TabFilter {
        ALL, GROUPS, UNREAD, ARCHIVED, CALLS
    }
    ...
    // Tab filter states
    private var currentTabFilter: TabFilter = TabFilter.ALL
    private var callHistoryAdapter: CallHistoryAdapter? = null
    private lateinit var tabAll: com.google.android.material.chip.Chip
    private lateinit var tabGroups: com.google.android.material.chip.Chip
    private lateinit var tabUnread: com.google.android.material.chip.Chip
    private lateinit var tabArchived: com.google.android.material.chip.Chip
    private lateinit var tabCalls: com.google.android.material.chip.Chip
    ...
    private fun setConversationList(list: List<ConversationModel>) {
        ...
        if (noFiltersActive && searchBehaviorSubject.value == false) {
            // Only show near future events if we're on ALL tab, otherwise respect current tab filter
            if (currentTabFilter == TabFilter.ALL) {
    ...
    fun applyFilter() {
        ...
        filterConversation()
        // Don't call updateDataSet here since filterConversation() already does it
    }
    ...
    fun filterConversation() {
        ...
        val unreadFilterOn = currentTabFilter == TabFilter.UNREAD && newItems.isEmpty()
        val archivedFilterOn = currentTabFilter == TabFilter.ARCHIVED && newItems.isEmpty()
        
        if ((archiveFilterOn && newItems.isEmpty()) || archivedFilterOn || unreadFilterOn) {
            binding.noArchivedConversationLayout.visibility = View.VISIBLE
            // Update the empty state message and icon based on current tab
            updateEmptyStateContent()
        } else {
            binding.noArchivedConversationLayout.visibility = View.GONE
        }

        // Update filterableConversationItems to maintain consistency
        setFilterableItems(newItems)
        
        // Update adapter with proper animation to prevent RecyclerView inconsistencies
    ...
    private fun hasCallHistory(conversation: ConversationModel): Boolean {
        // Check if conversation has call-related system messages
        return conversation.lastMessage?.systemMessageType in listOf(
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_STARTED,
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_JOINED,
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_LEFT,
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_ENDED,
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_ENDED_EVERYONE,
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_MISSED,
            com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_TRIED
        ) || conversation.hasCall || conversation.canStartCall
    }

    private fun loadCallHistory() {
        // Get all conversations with call history and convert to call history items
        val callHistoryItems = mutableListOf<CallHistoryItem>()
        
        // Get conversations from the current list that have call history
        val conversationsWithCalls = conversationItems
            .filterIsInstance<ConversationItem>()
            .map { it.model }
            .filter { hasCallHistory(it) }
        
        // For each conversation, create call history items based on their system messages
        conversationsWithCalls.forEach { conversation ->
            // Create a call history item for the last call-related message
            conversation.lastMessage?.let { message ->
                if (message.systemMessageType in listOf(
                    com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_STARTED,
                    com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_JOINED,
                    com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_LEFT,
                    com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_ENDED,
                    ...
                    val callType = when (message.systemMessageType) {
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_JOINED -> CallType.INCOMING
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_STARTED -> CallType.OUTGOING
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_MISSED -> CallType.MISSED
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_ENDED -> CallType.ENDED
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_ENDED_EVERYONE -> CallType.ENDED_EVERYONE
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_TRIED -> CallType.TRIED
                        com.nextcloud.talk.chat.data.model.ChatMessage.SystemMessageType.CALL_LEFT -> CallType.LEFT
                        else -> CallType.ENDED
                    }
                    
                    val contactName = conversation.displayName ?: conversation.name ?: "Unknown"
                    val profilePictureUrl = com.nextcloud.talk.utils.ApiUtils.getUrlForConversationAvatarWithVersion(
                        1,
                        currentUser?.baseUrl,
                        conversation.token,
                        false, // isDark
        ...
        callHistoryItems.sortByDescending { it.timestamp }
        
        // Update the adapter
        callHistoryAdapter?.updateCallHistory(callHistoryItems)
    }

    private fun navigateToConversation(conversationId: String) {
        // Navigate to the conversation
        val intent = Intent(this, ChatActivity::class.java)
        intent.putExtra(BundleKeys.KEY_ROOM_TOKEN, conversationId)
        startActivity(intent)
    }

    private fun startCall(conversationId: String) {
        // Start a call in the conversation
        val intent = Intent(this, ChatActivity::class.java)
        intent.putExtra(BundleKeys.KEY_ROOM_TOKEN, conversationId)
    ...
    private fun filter(conversation: ConversationModel): Boolean {
        ...
        when (currentTabFilter) {
            TabFilter.GROUPS -> {
                // Show only group conversations (not 1:1 or system)
                result = result && (conversation.type == ConversationEnums.ConversationType.ROOM_GROUP_CALL ||
                        conversation.type == ConversationEnums.ConversationType.ROOM_PUBLIC_CALL)
            }
            TabFilter.UNREAD -> {
                // Show only conversations with unread messages
                result = result && (conversation.unreadMessages > 0)
            }
            TabFilter.ARCHIVED -> {
                // Show only archived conversations
                result = result && conversation.hasArchived
            }
            TabFilter.CALLS -> {
                // Show only conversations with call history (call-related system messages)
                result = result && hasCallHistory(conversation)
            }
            TabFilter.ALL -> {
                // Show all conversations (handled by existing filter state logic below)
            }
        }
        
        // Apply existing filter state logic
        for ((k, v) in filterState) {
            if (v) {
                when (k) {
                    ...
                    UNREAD -> {
                        // Skip UNREAD filter when we're already in UNREAD tab since it's handled above
                        if (currentTabFilter != TabFilter.UNREAD) {
                            result = result && (conversation.unreadMessages > 0)
                        }
                    }
                    
                    ARCHIVE -> {
                        // Skip ARCHIVE filter when we're already in ARCHIVED tab since it's handled above
                        if (currentTabFilter != TabFilter.ARCHIVED) {
                            result = result && conversation.hasArchived
                        }
                    }
        ...
        Log.d(TAG, "Conversation: ${conversation.name} TabFilter: $currentTabFilter Result: $result")
        return result
    }
    ...
    private fun initTabFilters() {
        tabAll = findViewById(R.id.tab_all)
        tabGroups = findViewById(R.id.tab_groups)
        tabUnread = findViewById(R.id.tab_unread)
        tabArchived = findViewById(R.id.tab_archived)
        tabCalls = findViewById(R.id.tab_calls)

        // Set up tab click listeners
        tabAll.setOnClickListener { setActiveTab(TabFilter.ALL) }
        tabGroups.setOnClickListener { setActiveTab(TabFilter.GROUPS) }
        tabUnread.setOnClickListener { setActiveTab(TabFilter.UNREAD) }
        tabArchived.setOnClickListener { setActiveTab(TabFilter.ARCHIVED) }
        tabCalls.setOnClickListener { setActiveTab(TabFilter.CALLS) }

        // Set initial active tab
        setActiveTab(TabFilter.ALL)
    }

    private fun setActiveTab(tabFilter: TabFilter) {
        currentTabFilter = tabFilter

        // Update chip appearance
        updateTabAppearance(tabAll, tabFilter == TabFilter.ALL)
        updateTabAppearance(tabGroups, tabFilter == TabFilter.GROUPS)
        updateTabAppearance(tabUnread, tabFilter == TabFilter.UNREAD)
        updateTabAppearance(tabArchived, tabFilter == TabFilter.ARCHIVED)
        updateTabAppearance(tabCalls, tabFilter == TabFilter.CALLS)

        // Update floating action button based on selection
        updateFloatingActionButton(tabFilter)

        // Show/hide appropriate RecyclerView and load data
        when (tabFilter) {
            TabFilter.CALLS -> {
                binding.recyclerView.visibility = View.GONE
                binding.callHistoryRecyclerView.visibility = View.VISIBLE
                binding.noArchivedConversationLayout.visibility = View.GONE // Hide empty state for calls tab
                loadCallHistory()
            }
            TabFilter.ARCHIVED -> {
                binding.recyclerView.visibility = View.VISIBLE
                binding.callHistoryRecyclerView.visibility = View.GONE
                // Apply archived filter immediately using existing filtering logic
                // Save filter state to storage (same as filter dialog does)
                val accountId = UserIdUtils.getIdForUser(currentUser)
                arbitraryStorageManager.storeStorageSetting(accountId, UNREAD, "false", "")
                arbitraryStorageManager.storeStorageSetting(accountId, MENTION, "false", "")
                arbitraryStorageManager.storeStorageSetting(accountId, ARCHIVE, "true", "")
                ...
                // Apply the same filtering logic as the archive chip
                filterConversation()
            }
            else -> {
                binding.recyclerView.visibility = View.VISIBLE
                binding.callHistoryRecyclerView.visibility = View.GONE
                // Apply filtering for other tabs with a slight delay to prevent RecyclerView inconsistency
                Handler().postDelayed({
                    applyTabFilter()
                }, 50) // Small delay to ensure RecyclerView is ready
            }
        }
    }

    private fun updateTabAppearance(chip: com.google.android.material.chip.Chip, isSelected: Boolean) {
        if (isSelected) {
            chip.chipBackgroundColor = getColorStateList(R.color.colorPrimary)
            chip.setTextColor(getColor(R.color.white))
            chip.chipStrokeWidth = 0f
        } else {
            chip.chipBackgroundColor = getColorStateList(R.color.colorSecondaryLight)
            chip.setTextColor(getColor(R.color.grey_600))
            chip.chipStrokeWidth = 0f
    ...
    private fun applyTabFilter() {
        // Save filter state to storage when switching tabs
        val accountId = UserIdUtils.getIdForUser(currentUser)
        
        when (currentTabFilter) {
            TabFilter.ALL -> {
                // Show all conversations (no additional filtering)
                filterState[UNREAD] = false
                filterState[MENTION] = false
                filterState[ARCHIVE] = false
                arbitraryStorageManager.storeStorageSetting(accountId, UNREAD, "false", "")
                arbitraryStorageManager.storeStorageSetting(accountId, MENTION, "false", "")
                arbitraryStorageManager.storeStorageSetting(accountId, ARCHIVE, "false", "")
            }
            TabFilter.GROUPS -> {
            ...
            TabFilter.UNREAD -> {
                ...
                filterState[MENTION] = false
                filterState[ARCHIVE] = false
                arbitraryStorageManager.storeStorageSetting(accountId, UNREAD, "true", "")
                arbitraryStorageManager.storeStorageSetting(accountId, MENTION, "false", "")
                arbitraryStorageManager.storeStorageSetting(accountId, ARCHIVE, "false", "")
            }
            TabFilter.ARCHIVED -> {
            ...
            TabFilter.CALLS -> {
                // Show only conversations with calls
                filterState[UNREAD] = false
                filterState[MENTION] = false
                filterState[ARCHIVE] = false
                arbitraryStorageManager.storeStorageSetting(accountId, UNREAD, "false", "")
    ...
    private fun updateFloatingActionButton(tabFilter: TabFilter) {
        when (tabFilter) {
            TabFilter.ALL -> {
                binding.floatingActionButton.text = getString(R.string.chat)
                binding.floatingActionButton.visibility = View.VISIBLE
            }
            TabFilter.GROUPS -> {
                binding.floatingActionButton.text = getString(R.string.groups)
                binding.floatingActionButton.visibility = View.VISIBLE
            }
            TabFilter.UNREAD -> {
            ...
            TabFilter.CALLS -> {
                binding.floatingActionButton.text = getString(R.string.calls)
                binding.floatingActionButton.visibility = View.VISIBLE
            }
        }
    }
        ...
        if (showShareToScreen) {
            ...
            searchItem!!.setOnActionExpandListener(object : MenuItem.OnActionExpandListener {
                override fun onMenuItemActionExpand(item: MenuItem): Boolean {
                    ...
                    // Show the current tab's filtered conversations when search is opened
                    val currentFilteredItems = getCurrentFilteredConversations()
                    adapter!!.updateDataSet(currentFilteredItems, false)
                ...
                override fun onMenuItemActionCollapse(item: MenuItem): Boolean {
                    ...
                    searchBehaviorSubject.onNext(false)
                    // Return to the current tab's filtered conversations when search is closed
                    applyTabFilter()
    ...
    private fun prepareViews() {
        ...
        // Initialize call history adapter
        callHistoryAdapter = CallHistoryAdapter(
            onCallItemClick = { callItem ->
                // Navigate to conversation
                navigateToConversation(callItem.conversationId)
            },
            onPhoneIconClick = { callItem ->
                // Start a call
                startCall(callItem.conversationId)
            }
        )
        
        binding.callHistoryRecyclerView.layoutManager = androidx.recyclerview.widget.LinearLayoutManager(this)
        binding.callHistoryRecyclerView.adapter = callHistoryAdapter
        ...
        binding.floatingActionButton.setOnClickListener {
            when (currentTabFilter) {
                TabFilter.GROUPS -> {
                    // Launch ConversationCreationActivity for creating groups
                    val intent = Intent(this, ConversationCreationActivity::class.java)
                    startActivity(intent)
                }
                TabFilter.ALL, TabFilter.CALLS -> {
                    run(context)
                    showNewConversationsScreen()
                }
                TabFilter.UNREAD -> {
                    // This shouldn't happen as button is hidden for UNREAD
                }

                TabFilter.ARCHIVED -> TODO()
            }
        }
        binding.floatingActionButton.let { 
            viewThemeUtils.material.colorMaterialButtonContent(it, ColorRole.PRIMARY)
            it.setTextColor(getColor(R.color.white))
            it.iconTint = getColorStateList(R.color.white)
        }
        ...
        // Hide the filter conversations button since we now have tab filters
        //binding.filterConversationsButton.visibility = View.GONE
    ...
    private fun onQueryTextChange(newText: String?) {
        if (!TextUtils.isEmpty(searchQuery)) {
            ...
            // Always perform search (or reset) when text changes, including empty text
            performFilterAndSearch(newText)
        }
    }

    private fun getCurrentFilteredConversations(): MutableList<AbstractFlexibleItem<*>> {
        val filteredItems: MutableList<AbstractFlexibleItem<*>> = ArrayList()
        val items = conversationItems
    ...
    private fun searchWithinFilteredConversations(filteredItems: MutableList<AbstractFlexibleItem<*>>, searchQuery: String): MutableList<AbstractFlexibleItem<*>> {
        val searchResults: MutableList<AbstractFlexibleItem<*>> = ArrayList()
        val query = searchQuery.lowercase()
        
        for (item in filteredItems) {
            if (item is ConversationItem) {
                val conversation = item.model
                val displayName = conversation.displayName?.lowercase() ?: ""
                val lastMessage = conversation.lastMessage?.message?.lowercase() ?: ""
                
                // Check if the search query matches the conversation name or last message
                if (displayName.contains(query) || lastMessage.contains(query)) {
                    searchResults.add(item)
                }
            } else {
                // Keep non-conversation items (like headers) in the results
                searchResults.add(item)
            }
        }
    ...
    private fun searchCallHistory(searchQuery: String) {
        // Get all conversations with call history and convert to call history items
        val callHistoryItems = mutableListOf<CallHistoryItem>()
        ...
        // Filter by search query (search by contact name only)
        val filteredConversations = conversationsWithCalls.filter { conversation ->
            val displayName = conversation.displayName?.lowercase() ?: ""
            displayName.contains(searchQuery.lowercase())
        }
        
        // For each filtered conversation, create call history items
            ...
            conversation.lastMessage?.let { message ->
                if (message.systemMessageType in listOf(
                    ...
                    val callHistoryItem = CallHistoryItem(
                        ...
                        profilePictureUrl = ApiUtils.getUrlForConversationAvatarWithVersion(
                            1, // version
                            currentUser!!.baseUrl,
                            conversation.token,
                            false, // isDark
        ...
        // Update the call history adapter with filtered results
        callHistoryAdapter?.updateCallHistory(callHistoryItems)
    }
    ...
    private fun performFilterAndSearch(filter: String?) {
        if (filter.isNullOrEmpty() || filter.length < SEARCH_MIN_CHARS) {
            ...
            // Handle Calls tab differently - search call history
            if (currentTabFilter == TabFilter.CALLS) {
                searchCallHistory(filter)
            } else {
                // Get the currently filtered conversations based on the selected tab
                val currentFilteredItems = getCurrentFilteredConversations()
                
                // Search only within the current tab's filtered conversations
                val searchResults = searchWithinFilteredConversations(currentFilteredItems, filter)
    ...
    private fun resetSearchResults() {
        clearMessageSearchResults()
        // Reset to show the current tab's filtered conversations
        if (currentTabFilter == TabFilter.CALLS) {
            // Reload call history for Calls tab
            loadCallHistory()
        } else {
            applyTabFilter()
        }
    ...
    private fun updateEmptyStateContent() {
        when (currentTabFilter) {
            TabFilter.UNREAD -> {
               // binding.noArchivedConversationIcon.setImageResource(R.drawable.baseline_notifications_24)
                binding.noArchivedConversationText.text = getString(R.string.no_unread_conversations)
            }
            TabFilter.ARCHIVED -> {
                //binding.noArchivedConversationIcon.setImageResource(R.drawable.outline_archive_24)
                binding.noArchivedConversationText.text = getString(R.string.no_conversations_archived)
            }
            else -> {
                // Default to archived for other archive-related filters
                binding.noArchivedConversationIcon.setImageResource(R.drawable.outline_archive_24)
                binding.noArchivedConversationText.text = getString(R.string.no_conversations_archived)
            }
        }
