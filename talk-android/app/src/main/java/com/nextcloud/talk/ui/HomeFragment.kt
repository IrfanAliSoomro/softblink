...
import com.nextcloud.talk.activities.BaseActivity.AppBarLayoutType
...
class HomeFragment : Fragment(), FlexibleAdapter.OnItemClickListener, FlexibleAdapter.OnItemLongClickListener {
    ...
    // EventBus for real-time updates - same as ConversationsListActivity
    private val eventBus: org.greenrobot.eventbus.EventBus
        get() = (requireActivity() as BaseActivity).eventBus
    
    // Simple periodic refresh for conversations
    private var refreshHandler: Handler? = null
    private val refreshRunnable = object : Runnable {
        override fun run() {
            if (!isRefreshing && currentUser != null && !isDetached) {
                Log.d(TAG, "Periodic refresh triggered")
                fetchRooms()
            }
            // Schedule next refresh in 30 seconds
            if (!isDetached && currentUser != null) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Log.d(TAG, "Fragment attached: ${isAdded}, Activity: ${activity?.javaClass?.simpleName}")
        ...
        //     // Filter your list or perform search here
        //     onQueryTextChange_(query)
        // }
    ...
    override fun onDestroyView() {
        ...
        val eventBus = (requireActivity() as? BaseActivity)?.eventBus
        eventBus?.let {
    ...
    override fun onResume() {
        ...
        // Register for events to refresh conversations on new messages - same as ConversationsListActivity
        if (!eventBus.isRegistered(this)) {
            eventBus.register(this)
            Log.d(TAG, "EventBus registered successfully")
        } else {
            Log.d(TAG, "EventBus already registered")
        }
        Log.d(TAG, "EventBus registration check complete")
        
        // Verify EventBus is working
        Log.d(TAG, "EventBus isRegistered: ${eventBus.isRegistered(this)}")
        Log.d(TAG, "EventBus hasSubscriberForEvent: ${eventBus.hasSubscriberForEvent(EventStatus::class.java)}")
        
        // Test EventBus accessibility
        try {
            Log.d(TAG, "EventBus accessible: true")
            Log.d(TAG, "EventBus toString: ${eventBus.toString()}")
        } catch (e: Exception) {
            Log.e(TAG, "EventBus not accessible", e)
        }
        
        // Force refresh when resuming to ensure latest data
        if (currentUser != null) {
            Handler().postDelayed({
                Log.d(TAG, "Forcing refresh on resume")
                fetchRooms()
            }, 1000) // 1 second delay
        }
        
        // Test EventBus by posting a test event
        Handler().postDelayed({
            Log.d(TAG, "Testing EventBus with test event")
            try {
                val testEvent = ConversationsListFetchDataEvent()
                eventBus.post(testEvent)
                Log.d(TAG, "Test event posted successfully")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to post test event", e)
            }
        }, 2000) // 2 second delay
    ...
    private fun initObservers() {
        ...
        conversationsListViewModel.getRoomsViewState.observe(this) { state ->
            when (state) {
                ...
                is ConversationsListViewModel.GetRoomsErrorState -> {
                    ...
                    // Reset refreshing state even on error
                    isRefreshing = false
                    binding.swipeRefreshLayoutView.isRefreshing = false
                }
        ...
        lifecycleScope.launch {
            conversationsListViewModel.getRoomsFlow
                .onEach { list ->
                     // Always update the conversation list when new data arrives
    ...
    private fun handleNoteToSelfShortcut(noteToSelfAvailable: Boolean, noteToSelfToken: String) {
        if (noteToSelfAvailable) {
            ...
            val shortcut = ShortcutInfoCompat.Builder(requireContext(), NOTE_TO_SELF_SHORTCUT_ID)
                .setShortLabel(openNotesString)
                .setLongLabel(openNotesString)
                .setIcon(IconCompat.createWithResource(requireContext(), R.drawable.ic_pencil_grey600_24dp))
                .setIntent(intent)
                .build()

            ShortcutManagerCompat.pushDynamicShortcut(requireContext(), shortcut)
        } else {
            ShortcutManagerCompat.removeDynamicShortcuts(requireContext(), listOf(NOTE_TO_SELF_SHORTCUT_ID))
        }
    }

    // private fun setConversationList(list: List<ConversationModel>) {
    //     // Only keep group conversations
    //     val groupConversations = list.filter {
    //         it.type == ConversationEnums.ConversationType.ROOM_PUBLIC_CALL
    //     }
    //     // ROOM_TYPE_ONE_TO_ONE_CALL -- Only chat
    //     // Group
    ...
    private fun setConversationList(list: List<ConversationModel>) {
        Log.d(TAG, "setConversationList called with ${list.size} conversations")
        
        // Only keep group conversations
        val groupConversations = list.filter {
            it.type != ConversationEnums.ConversationType.ROOM_GROUP_CALL
        }
        Log.d(TAG, "Filtered to ${groupConversations.size} group conversations")
        ...
        Log.d(TAG, "Processed conversations - items: ${conversationItems.size}, withHeader: ${conversationItemsWithHeader.size}, nearFuture: ${nearFutureEventConversationItems.size}")
        ...
        if (noFiltersActive && searchBehaviorSubject.value == false) {
            Log.d(TAG, "Updating adapter with nearFutureEventConversationItems")
            adapter?.updateDataSet(nearFutureEventConversationItems, false)
            // Force adapter to refresh and notify changes
            adapter?.notifyDataSetChanged()
            // Additional force refresh to ensure UI updates
            forceAdapterRefresh()
        } else {
            Log.d(TAG, "Applying filter to conversation items")
            applyFilter()
        }

        Handler().postDelayed({ checkToShowUnreadBubble() }, GroupFragment.Companion.UNREAD_BUBBLE_DELAY.toLong())
    ...
    private fun addToNearFutureEventConversationItems(conversation: ConversationModel) {
        val conversationItem = ConversationItem(conversation, currentUser!!, requireContext(), null, viewThemeUtils)
    ...
    private fun initSearchView() {
        ...
        if (searchItem != null) {
            ...
            val appPreferences = (requireActivity() as? BaseActivity)?.appPreferences
                appPreferences?.let {
                    if (it.isKeyboardIncognito) {
    ...
    private fun initSearchDisposable() {
        if (searchViewDisposable == null || searchViewDisposable?.isDisposed == true) {
            searchViewDisposable = observeSearchView(searchView!!)
                ...
                .subscribe { newText: String? -> onQueryTextChange_(newText) }
        }
    }
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        ...
        if (searchItem != null) {
            ...
            // Optionally log or handle the missing menu item
            Log.e("HomeFragment", "action_search menu item not found!")
        }
        ...
        if (showShareToScreen) {
            hideSearchBar()
            (activity as? BaseActivity)?.supportActionBar?.setTitle(R.string.send_to_three_dots)
        } else if (forwardMessage) {
            hideSearchBar()
            (activity as? BaseActivity)?.supportActionBar?.setTitle(R.string.nc_forward_to_three_dots)
        } else {
            ...
            //     showSearchView(searchView, searchItem)
            //     viewThemeUtils.platform.themeStatusBar(requireActivity())
            // }
            searchView!!.findViewById<View>(R.id.search_close_btn).setOnClickListener {
                if (TextUtils.isEmpty(searchView!!.query.toString())) {
                    ...
                    viewThemeUtils.platform.resetStatusBar(requireActivity())
                } else {
                    resetSearchResults()
    ...
    private fun hasActivityActionSendIntent(): Boolean =
        Intent.ACTION_SEND == requireActivity().intent.action || Intent.ACTION_SEND_MULTIPLE == requireActivity()
            .intent.action
    ...
    fun fetchRooms() {
        Log.d(TAG, "fetchRooms called")

        // Add null safety checks
        if (!isAdded || isDetached) {
            Log.d(TAG, "Fragment not attached, ignoring fetchRooms call")
            return
        }
        
        if (currentUser == null) {
            Log.w(TAG, "currentUser is null, ignoring fetchRooms call")
            return
        }

        // Set refreshing state to prevent multiple simultaneous requests
        isRefreshing = true
        Log.d(TAG, "Setting refreshing state to true")
        
        try {
            binding.swipeRefreshLayoutView.isRefreshing = true
            Log.d(TAG, "Swipe refresh set to true")
        } catch (e: Exception) {
            Log.e(TAG, "Error setting swipe refresh", e)
        }

        try {
            Log.d(TAG, "Calling conversationsListViewModel.getRooms()")
            conversationsListViewModel.getRooms()
        } catch (e: Exception) {
            Log.e(TAG, "Error in fetchRooms", e)
            isRefreshing = false
            try {
                binding.swipeRefreshLayoutView.isRefreshing = false
            } catch (e2: Exception) {
                Log.e(TAG, "Error resetting swipe refresh", e2)
            }
        }
    }
    
    /**
     * Test method to manually trigger refresh - can be called from HomeScreen
     */
    fun testRefresh() {
        ...
        try {
            ...
            Log.e(TAG, "Error in testRefresh", e)
        }
    }
    
    /**
     * Force refresh the adapter to show latest data
     */
    fun forceAdapterRefresh() {
        Log.d(TAG, "Force adapter refresh called")
        try {
            if (adapter != null) {
                adapter?.notifyDataSetChanged()
                Log.d(TAG, "Adapter notified of data change")
            } else {
                Log.w(TAG, "Adapter is null, cannot refresh")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error forcing adapter refresh", e)
        }
    }
    ...
    fun checkEventBusStatus() {
        Log.d(TAG, "Checking EventBus status")
        
        // Add null safety checks
        if (!isAdded || isDetached) {
            Log.d(TAG, "Fragment not attached, cannot check EventBus status")
            return
        }
        
        try {
            val isRegistered = eventBus.isRegistered(this)
            Log.d(TAG, "EventBus isRegistered: $isRegistered")
            
            if (isRegistered) {
                Log.d(TAG, "EventBus registration successful")
            } else {
                Log.d(TAG, "EventBus not registered, attempting to register")
                eventBus.register(this)
                Log.d(TAG, "EventBus registration attempted")
            }
        } catch (e: Exception) {
            Log.e(TAG, "EventBus status check failed", e)
        }
    }
    ...
    private fun fetchPendingInvitations() {
        if (hasSpreedFeatureCapability(currentUser!!.capabilities!!.spreedCapability!!, SpreedFeatures.FEDERATION_V1)) {
            binding.conversationListHintInclude.conversationListHintLayout.setOnClickListener {
                val intent = Intent(requireActivity(), InvitationsActivity::class.java)
                startActivity(intent)
            }
    ...
    private fun addToConversationItems(conversation: ConversationModel) {
        if (requireActivity().intent.getStringExtra(KEY_FORWARD_HIDE_SOURCE_ROOM) != null &&
            requireActivity().intent.getStringExtra(KEY_FORWARD_HIDE_SOURCE_ROOM) == conversation.token
        ) {
            return
        }
    ...
    private fun prepareViews() {
        ...
        // Add divider lines between conversation items
        val dividerDecoration = DividerItemDecoration(requireContext(), DividerItemDecoration.VERTICAL)
        dividerDecoration.setDrawable(ContextCompat.getDrawable(requireContext(), R.drawable.conversation_divider)!!)
        ...
        binding.recyclerView.setOnTouchListener { v: View, _: MotionEvent? ->
            // if (!isDestroyed) {
                // val imm = getSystemService(requireContext().INPUT_METHOD_SERVICE) as InputMethodManager
        ...
        binding.swipeRefreshLayoutView.setOnRefreshListener {
            showWarning(false, "")
            Log.d(TAG, "Swipe refresh triggered manually")
            fetchRooms()
            fetchPendingInvitations()
        }
    ...
    private fun hideLogoForBrandedClients() {
        if (!BrandingUtils.isOriginalNextcloudClient(requireContext())) {
                    ...
                    for (flexItem in conversationItems) {
                        ...
                        if (position != null && hasUnreadItems(conversation) && position > lastVisibleItem) {
                            ...
                            if (!binding.newMentionPopupBubble.isShown) {
                                ...
                                val popupAnimation = AnimationUtils.loadAnimation(requireActivity(), R.anim
                                    .popup_animation)
    ...
    private fun onQueryTextChange_(newText: String?) {
        if (!TextUtils.isEmpty(searchQuery)) {
        ...
        if (selectedConversation != null) {
            ...
            if (showShareToScreen) {
                ...
                if (hasChatPermission && !isReadOnlyConversation(selectedConversation!!)) {
                    openConversation(requireActivity().intent.getStringExtra(KEY_FORWARD_MSG_TEXT))
    ...
    private fun handleSharedData() {
        ...
        if (textToPaste!!.isNotEmpty()) {
            ...
            Snackbar
                .make(binding.root, requireActivity().resources.getString(R.string.nc_common_error_sorry),
    ...
    private fun collectDataFromIntent() {
        ...
        if (requireActivity().intent != null) {
            if (Intent.ACTION_SEND == requireActivity().intent.action || Intent.ACTION_SEND_MULTIPLE == requireActivity().intent.action) {
                try {
                    val mimeType = requireActivity().intent.type
                    if (Mimetype.TEXT_PLAIN == mimeType && requireActivity().intent.getStringExtra(Intent.EXTRA_TEXT) != null) {
                        ...
                        textToPaste = requireActivity().intent.getStringExtra(Intent.EXTRA_TEXT)
                    } else {
                        if (requireActivity().intent.clipData != null) {
                            for (i in 0 until requireActivity().intent.clipData!!.itemCount) {
                                val item = requireActivity().intent.clipData!!.getItemAt(i)
                                if (item.uri != null) {
                            ...
                            filesToShare!!.add(requireActivity().intent.data.toString())
                        }
                    }
                    if (filesToShare!!.isEmpty() && textToPaste!!.isEmpty()) {
                        Snackbar.make(
                            binding.root,
                            requireActivity().resources.getString(R.string.nc_common_error_sorry),
                            Snackbar.LENGTH_LONG
                        ).show()
    ...
    private fun upload() {
        ...
        try {
            ...
            Snackbar.make(binding.root, requireActivity().resources.getString(R.string.nc_upload_failed), Snackbar.LENGTH_LONG)
                .show()
        ...
        when (requestCode) {
            UploadAndShareFilesWorker.REQUEST_PERMISSION -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Snackbar.make(
                        binding.root,
                        requireActivity().getString(R.string.read_storage_no_permission),
                        Snackbar.LENGTH_LONG
                    ).show()
                }
            }
                ...
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (!PowerManagerUtils().isIgnoringBatteryOptimizations() &&
                        ...
                        val dialogText = String.format(
                            requireActivity().resources.getString(R.string.nc_ignore_battery_optimization_dialog_text),
                            requireActivity().resources.getString(R.string.nc_app_name)
                        )
    ...
    private fun shouldShowNotificationWarning(): Boolean {
        ...
        val messagesChannelNotEnabled = !NotificationUtils.isMessagesNotificationChannelEnabled(requireContext())
        val callsChannelNotEnabled = !NotificationUtils.isCallsNotificationChannelEnabled(requireContext())
    ...
    private fun openConversation(textToPaste: String? = "") {
        if (CallActivity.active &&
            ...
            Snackbar.make(
                binding.root,
                context?.getString(R.string.restrict_join_other_room_while_call) ?: "Cannot join other room while call is active",
                Snackbar.LENGTH_LONG
            ).show()
            return
        }
    ...
    private fun openConversationTemp(textToPaste: String? = "") {
        if (CallActivity.active &&
            selectedConversation!!.token != ApplicationWideCurrentRoomHolder.getInstance().currentRoomToken
        ) {
            Snackbar.make(
                binding.root,
                requireActivity().getString(R.string.restrict_join_other_room_while_call),
                Snackbar.LENGTH_LONG
            ).show()
            return
        }
        ...
        val intent = Intent(requireContext(), ChatActivity::class.java)
        intent.putExtra("selectedTab", "chat")
        intent.putExtras(bundle)
        startActivity(intent)
    ...
    fun onMessageEvent(eventStatus: EventStatus) {
        Log.d(TAG, "EventStatus received: ${eventStatus.eventType} for user: ${eventStatus.userId}")
        ...
        if (currentUser != null && eventStatus.userId == currentUser!!.id) {
            when (eventStatus.eventType) {
                EventStatus.EventType.CONVERSATION_UPDATE -> if (eventStatus.isAllGood && !isRefreshing) {
                    Log.d(TAG, "Conversation update event received, refreshing conversations")
                    fetchRooms()
                }

                else -> {
                    Log.d(TAG, "Other EventStatus event type: ${eventStatus.eventType}")
                }
            }
        } else {
            Log.d(TAG, "Event ignored - currentUser: ${currentUser != null}, userId match: ${currentUser?.id == eventStatus.userId}")
        }
    }
    ...
    //     // Add null safety checks
    //     if (!isAdded || isDetached) {
    //         Log.d(TAG, "Fragment not attached, ignoring EventStatus event")
    //         return
    //     }
    //
    //     if (currentUser != null && eventStatus.userId == currentUser!!.id) {
    ...
    //                 Log.d(TAG, "Participants update event received, refreshing conversations")
    //                 try {
    //                     requireActivity().runOnUiThread {
    //                         if (isAdded && !isDetached) {
    //                             fetchRooms()
    ...
    //                 Log.d(TAG, "Push registration event received")
    ...
    //             }
    //             else -> {
    //                 Log.d(TAG, "Other event type: ${eventStatus.eventType}")
    //             }
    //         }
    //     } else {
    //         Log.d(TAG, "Event ignored - currentUser: ${currentUser != null}, userId match: ${currentUser?.id == eventStatus.userId}")
    ...
    /**
     * Catch-all event handler to see what events are being sent
     */
    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onAnyEvent(event: Any) {
        // Add null safety checks
        if (!isAdded || isDetached) {
            Log.d(TAG, "Fragment not attached, ignoring catch-all event")
            return
        }
        
        Log.d(TAG, "Any event received: ${event.javaClass.simpleName}")
        when (event) {
            is EventStatus -> Log.d(TAG, "EventStatus via catch-all: ${event.eventType}")
            is WebSocketCommunicationEvent -> Log.d(TAG, "WebSocket via catch-all: ${event.type}")
            is ConversationsListFetchDataEvent -> Log.d(TAG, "ConversationsListFetchDataEvent via catch-all")
            else -> Log.d(TAG, "Unknown event type: ${event.javaClass.name}")
        }
    }
    
    /**
     * Lifecycle-aware event handler for real-time updates
     */
    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onRealTimeEvent(event: Any) {
        if (!isAdded || isDetached) {
            Log.d(TAG, "Fragment not attached, ignoring event: ${event.javaClass.simpleName}")
            return
        }
        
        Log.d(TAG, "Real-time event received: ${event.javaClass.simpleName}")
        when (event) {
            is EventStatus -> {
                Log.d(TAG, "Real-time EventStatus: ${event.eventType}")
                if (event.eventType == EventStatus.EventType.CONVERSATION_UPDATE && event.isAllGood) {
                    Log.d(TAG, "Triggering refresh from real-time event")
                    try {
                        fetchRooms()
                    } catch (e: Exception) {
                        Log.e(TAG, "Error refreshing from real-time EventStatus event", e)
                    }
                }
            }
            is WebSocketCommunicationEvent -> {
                Log.d(TAG, "Real-time WebSocket: ${event.type}")
                if (event.type in listOf("chat", "message", "conversation")) {
                    Log.d(TAG, "Triggering refresh from WebSocket event")
                    try {
                        fetchRooms()
                    } catch (e: Exception) {
                        Log.e(TAG, "Error refreshing from real-time WebSocket event", e)
                    }
                }
            }
        }
    ...
    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onConversationsListFetchDataEvent(conversationsListFetchDataEvent: ConversationsListFetchDataEvent?) {
        // Add null safety checks
        if (!isAdded || isDetached) {
            Log.d(TAG, "Fragment not attached, ignoring ConversationsListFetchDataEvent")
            return
        }
        
        Log.d(TAG, "ConversationsListFetchDataEvent received")
        try {
            fetchRooms()
        } catch (e: Exception) {
            Log.e(TAG, "Error refreshing from ConversationsListFetchDataEvent", e)
        }
        // Note: conversationsListBottomDialog is not used in this fragment
    }
     ...
     */
    @Subscribe(threadMode = ThreadMode.MAIN)
    fun onWebSocketMessageEvent(event: WebSocketCommunicationEvent?) {
        // Add null safety checks
        if (!isAdded || isDetached) {
            Log.d(TAG, "Fragment not attached, ignoring WebSocket event")
            return
        }
        
        event?.let {
            Log.d(TAG, "WebSocket event received: ${it.type}")
            when (it.type) {
                "chat", "message", "conversation", "refreshChat" -> {
                    Log.d(TAG, "Message/chat event received, refreshing conversations")
                    try {
                        fetchRooms()
                    } catch (e: Exception) {
                        Log.e(TAG, "Error refreshing from WebSocket event", e)
                    }
                }
                "typing" -> {
                    Log.d(TAG, "Typing event received - not refreshing")
                }
                else -> {
                    Log.d(TAG, "Other WebSocket event: ${it.type} - refreshing to be safe")
                    try {
                        fetchRooms()
                    } catch (e: Exception) {
                        Log.e(TAG, "Error refreshing from WebSocket event", e)
    ...
    private fun deleteUserAndRestartApp() {
        ...
        WorkManager.getInstance(requireActivity().applicationContext).enqueue(accountRemovalWork)

        // Use lifecycle-aware observer instead of observeForever
        WorkManager.getInstance(requireActivity()).getWorkInfoByIdLiveData(accountRemovalWork.id)
            .observe(viewLifecycleOwner) { workInfo: WorkInfo? ->

                when (workInfo?.state) {
                    WorkInfo.State.SUCCEEDED -> {
                        val text = String.format(
                            resources.getString(R.string.nc_deleted_user),
    ...
    private fun deleteConversation(conversation: ConversationModel) {
        ...
        WorkManager.getInstance(requireActivity()).getWorkInfoByIdLiveData(deleteConversationWorker.id)
            .observe(viewLifecycleOwner) { workInfo: WorkInfo? ->
                if (workInfo != null) {
                    when (workInfo.state) {
                        WorkInfo.State.SUCCEEDED -> {
