...
@interface RoomsTableViewController () <CCCertificateDelegate, UISearchBarDelegate, UISearchControllerDelegate, UISearchResultsUpdating, UserStatusViewDelegate>
{
    ...
    UISegmentedControl *_topFilterControl;
    NSInteger _topFilterIndex;
...
- (void)viewDidLoad
{
    ...
    // Top filter segmented control (All, Groups, Unread, Archived, Calls)
    _topFilterIndex = 0;
    NSArray *segments = @[
        NSLocalizedString(@"All", @"All conversations"),
        NSLocalizedString(@"Groups", @"Group conversations"),
        NSLocalizedString(@"Unread", @"Unread conversations"),
        NSLocalizedString(@"Archived", @"Archived conversations"),
        NSLocalizedString(@"Calls", @"Conversations with calls")
    ];
    _topFilterControl = [[UISegmentedControl alloc] initWithItems:segments];
    _topFilterControl.selectedSegmentIndex = _topFilterIndex;
    _topFilterControl.translatesAutoresizingMaskIntoConstraints = NO;
    _topFilterControl.selectedSegmentTintColor = [NCAppBranding elementColor];
    [_topFilterControl setTitleTextAttributes:@{
        NSForegroundColorAttributeName: [NCAppBranding themeTextColor]
    } forState:UIControlStateSelected];
    [_topFilterControl addTarget:self action:@selector(topFilterChanged:) forControlEvents:UIControlEventValueChanged];
    ...
    [headerView addSubview:_topFilterControl];

    [NSLayoutConstraint activateConstraints:@[
        [_topFilterControl.leadingAnchor constraintEqualToAnchor:headerView.leadingAnchor constant:16],
        [_topFilterControl.trailingAnchor constraintEqualToAnchor:headerView.trailingAnchor constant:-16],
        [_topFilterControl.centerYAnchor constraintEqualToAnchor:headerView.centerYAnchor]
    ]];
...
- (void)setupSearchBar
{
    ...
    if (@available(iOS 26, *)) {
        if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPhone) {
            ...
            // We don't show the iOS toolbar filter button, as filters are handled by the top tabs.
...
- (void)updateAccountPickerMenu
{
    ...
    // "Add account" option removed for single-account branded app.
...
- (void)filterRooms
{
    ...
    // Apply additional top segmented filter (All, Groups, Unread, Archived, Calls)
    switch (_topFilterIndex) {
        case 1: { // Groups
            NSPredicate *predicate = [NSPredicate predicateWithFormat:@"type == %ld", kNCRoomTypeGroup];
            filteredRooms = [filteredRooms filteredArrayUsingPredicate:predicate];
            break;
        }
        case 2: { // Unread
            NSPredicate *predicate = [NSPredicate predicateWithFormat:@"unreadMessages > 0"];
            filteredRooms = [filteredRooms filteredArrayUsingPredicate:predicate];
            break;
        }
        case 4: { // Calls
            NSPredicate *predicate = [NSPredicate predicateWithFormat:@"hasCall == YES"];
            filteredRooms = [filteredRooms filteredArrayUsingPredicate:predicate];
            break;
        }
        default:
...
#pragma mark - Top filter segmented control

- (void)topFilterChanged:(UISegmentedControl *)sender
{
    _topFilterIndex = sender.selectedSegmentIndex;

    // Archived tab toggles between archived and normal conversations
    if (_topFilterIndex == 3) {
        _showingArchivedRooms = YES;
    } else {
        _showingArchivedRooms = NO;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == kRoomsSectionArchivedConversations) {
        // Archived conversations row is now handled by the top filter segmented control, so hide this cell.
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.tableView && indexPath.section == kRoomsSectionArchivedConversations) {
        // Archived conversations toggle is now controlled via the top filter segmented control.
        return;
    }
