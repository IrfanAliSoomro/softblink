...
class LoginViewController: UIViewController, UITextFieldDelegate, CCCertificateDelegate, DetailedOptionsSelectorTableViewControllerDelegate, AuthenticationViewControllerDelegate, QRScannerViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // MARK: - Hardcoded server URL
        // If you want to always use a specific server and skip the manual server entry screen,
        // set the URL here and automatically start the login flow.
        let hardcodedServerURL = "https://nc.softblinktech.com/"
        serverTextField.text = hardcodedServerURL
        serverTextField.isEnabled = false
        qrCodeButton.isHidden = true
        importAccountButton.isHidden = true

        // Automatically start login with the hardcoded server URL
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.startLoginProcess(serverURL: hardcodedServerURL, user: nil)
        }
